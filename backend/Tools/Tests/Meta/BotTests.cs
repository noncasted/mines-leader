...
public class BotFactoryWorkflowTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    [Fact]
    public async Task BotCreation_InitializesUserGrain() {
        ...
        await RunTransaction(async () => { state = await GetGrain<IUser>(id).GetState(); });
    ...
    public async Task BotCreation_InitializesDeck() {
        ...
        await RunTransaction(async () => { state = await GetGrain<IUserDeck>(id).GetState(); });
    ...
    public async Task BotCreation_RegistersAuth() {
        ...
        var exists = false;
        await RunTransaction(async () => { exists = await GetGrain<IUserAuth>(id).IsExists(); });
    ...
    public async Task BotCreation_CustomDeckFromBotPool() {
        ...
        await RunTransaction(async () => { selected = await GetGrain<IUserDeck>(id).GetSelected(); });
    ...
    public async Task BotCreation_FullWorkflow_InitializesAllGrains() {
        ...
        await RunTransaction(async () => { userState = await GetGrain<IUser>(id).GetState(); });
        ...
        await RunTransaction(async () => { deck = await GetGrain<IUserDeck>(id).GetSelected(); });
