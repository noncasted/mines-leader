...
public class ChainReactionTests {
    ...
    public void Use_ChainsFromInitialMine() {
        ...
        actionData.SpawnedMines.Should().Contain(pos =>
            chainTargets.Any(t => pos.DistanceTo(t) <= CardConfigs.ChainReaction.SpawnSize),
    ...
    public void Use_MaxChainLimitsSpread() {
        ...
        var maxSpawnPerTarget = PatternShapes.Rhombus(CardConfigs.ChainReaction.SpawnSize)
            .Positions.SelectMany(r => r).Count(v => v);
        actionData.SpawnedMines.Count.Should().BeLessThanOrEqualTo(
            CardConfigs.ChainReaction.MaxChain * maxSpawnPerTarget,
    ...
    public void Use_SpawnsMinesAroundTargets() {
        ...
        foreach (var pos in actionData.SpawnedMines) {
            ...
            board.Cells[pos].Should().BeAssignableTo<ITakenCell>()
                .Which.HasMine.Should().BeTrue($"spawned mine at {pos} should have mine");
        }
    }
    ...
    public void Use_SkipsExistingMines() {
        ...
        foreach (var pos in actionData!.SpawnedMines) {
            existingMines.Should().NotContain(pos,
    ...
    public void Use_FlaggedMinesSkippedInChain() {
        ...
        foreach (var pos in actionData.SpawnedMines) {
            ...
            pos.DistanceTo(new Position(3, 4)).Should().BeLessThanOrEqualTo(
                CardConfigs.ChainReaction.SpawnSize,
