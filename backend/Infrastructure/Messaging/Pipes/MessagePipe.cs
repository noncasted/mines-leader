...
public class MessagePipe : Grain, IMessagePipe
{
    ...
    {
        _logger.LogTrace("[Messaging] [Pipe] Binding observer to pipe {PipeId}", this.GetPrimaryKeyString());
        _observer = observer;
    ...
    {
        _logger.LogTrace("[Messaging] [Pipe] Sending one-way message {MessageType} to pipe {PipeId}",
