...
public class MetricsSnapshotService : IHostedService, IDisposable {
    ...
    public Task StartAsync(CancellationToken cancellationToken) {
        _serviceName = Environment.GetEnvironmentVariable("SERVICE_NAME")
            ?? System.Reflection.Assembly.GetEntryAssembly()?.GetName().Name?.ToLowerInvariant()
            ?? "unknown";
    ...
    private void OnMeasurement(Instrument instrument, int value, ReadOnlySpan<KeyValuePair<string, object?>> tags, object? state) {
        Record(instrument, value);
    }
