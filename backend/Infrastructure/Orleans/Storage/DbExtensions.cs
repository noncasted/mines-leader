...
public static class DbExtensions
{
    ...
    extension(IDbCommand command)
    {
        ...
        public IDbDataParameter CreateParameter<T>(
        ...
        {
            command.Parameters.Add(command.CreateParameter(ParameterDirection.Input, parameterName, value));
        }
    }
    ...
    extension(IDataRecord record)
    {
        ...
        public TValue GetValueOrDefault<TValue>(string fieldName, TValue @default)
        {
            try
            {
        ...
        public DateTime? GetDateTimeValueOrDefault(string fieldName, DateTime? @default = default)
        {
            try
            {
                var ordinal = record.GetOrdinal(fieldName);
    ...
    public static TValue GetValueOrDefault<TValue>(this IDataRecord record, int ordinal, TValue @default)
    {
        return record.IsDBNull(ordinal) ? @default : (TValue)record.GetValue(ordinal);
    }
    ...
    extension(IDataRecord record)
    {
        ...
        /// <returns>Key in the given field indicated by <see paramref="fieldName"/>.</returns>
        /// <exception cref="DataException"/>
        /// <remarks>This function throws if the given <see paramref="fieldName"/> does not exist.</remarks>        
        ...
        /// </summary>
        /// <param name="fieldName">The name of the field.</param>
        /// <returns>DateTime Key in the given field.</returns>
        /// <exception cref="DataException"/>
        /// <remarks>An explicit function like this is needed in cases where to connector infers a type that is undesirable.
        /// An example here is Npgsql.NodaTime, which makes Npgsql return Noda type and consequently Orleans is not able to
        /// use it since it expects .NET <see cref="System.DateTime"/>. This function throws if the given <see paramref="fieldName"/> does not exist.</remarks>
