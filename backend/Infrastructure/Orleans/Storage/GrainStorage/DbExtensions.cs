...
public static class DbExtensions
{
    ...
    /// </summary>
    private static readonly ReadOnlyDictionary<Type, DbType> typeMap = new ReadOnlyDictionary<Type, DbType>(
    ...
    public static IDbDataParameter CreateParameter<T>(
        this IDbCommand command, ParameterDirection direction, string parameterName, T value, int? size = null,
        DbType? dbType = null)
    {
    ...
    public static void AddParameter<T>(
        this IDbCommand command, string parameterName, T value, ParameterDirection direction = ParameterDirection.Input,
        int? size = null, DbType? dbType = null)
    {
    ...
    public static DateTime? GetDateTimeValueOrDefault(
        this IDataRecord record, string fieldName, DateTime? @default = default)
    {
        try
        {
            var ordinal = record.GetOrdinal(fieldName);
            return record.IsDBNull(ordinal)
                ? @default
                : DateTime.SpecifyKind(record.GetDateTime(ordinal), DateTimeKind.Utc);
        }
        catch (IndexOutOfRangeException e)
        {
            throw new DataException($"Field '{fieldName}' not found in data record.", e);
    ...
    public static async Task<TValue> GetValueOrDefaultAsync<TValue>(
        this DbDataReader record, string fieldName, TValue @default = default)
    {
        try
        {
            var ordinal = record.GetOrdinal(fieldName);
    ...
    /// <exception cref="IndexOutOfRangeException"/>                
    public static async Task<TValue> GetValueOrDefaultAsync<TValue>(
        this DbDataReader record, int ordinal, TValue @default = default)
    {
        return (await record.IsDBNullAsync(ordinal).ConfigureAwait(false))
            ? @default
            : (await record.GetFieldValueAsync<TValue>(ordinal).ConfigureAwait(false));
    ...
    public static async Task<TValue> GetValueAsync<TValue>(
        this DbDataReader record, string fieldName, CancellationToken cancellationToken = default)
    {
        try
        {
            var ordinal = record.GetOrdinal(fieldName);
    ...
    public static void ReflectionParameterProvider<T>(
        this IDbCommand command, T parameters, IReadOnlyDictionary<string, string> nameMap = null)
    {
        if (!EqualityComparer<T>.Default.Equals(parameters, default))
        {
            ...
            for (int i = 0; i < properties.Length; ++i)
            {
                ...
                parameter.ParameterName = nameMap != null && nameMap.ContainsKey(properties[i].Name)
