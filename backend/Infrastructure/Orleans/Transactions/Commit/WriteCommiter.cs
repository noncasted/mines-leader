...
public class WriteCommiter
{
    ...
    public WriteCommiter(IGrainFactory grains, ILogger logger)
    {
        _grains = grains;
        _logger = logger;
    }
        ...
        TransactionRunOptions options)
    {
        ...
        try
        {
            ...
            {
                var handle = _grains.GetGrain<ITransactionHandle>(Guid.Parse(participants.Info.Id));
                await handle.OnSuccess();
            }
