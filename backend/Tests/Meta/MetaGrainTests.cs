...
public class UserRatingTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    [Fact]
    public async Task AddRecord_WinRecord_IncreasesTotal() {
        ...
        var total = 0;
        await RunTransaction(async () => { total = await grain.GetTotal(); });
        total.Should().Be(25);
    }
    ...
    public async Task AddRecord_StatePersistsAcrossReferences() {
        ...
        var total = 0;
        await RunTransaction(async () => { total = await grain2.GetTotal(); });
        total.Should().Be(25);
    }
}

[Collection(nameof(OrleansIntegrationCollection))]
public class UserProgressionTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    [Fact]
    public async Task AddRecord_WinRecord_AddsExperience() {
        var id = Guid.NewGuid();
        var grain = GetGrain<IUserProgression>(id);
        await RunTransaction(() => grain.AddRecord(new UserProgressionRecords.Win { Date = DateTime.UtcNow, Experience = 100 }));
        var total = 0;
        await RunTransaction(async () => { total = await grain.GetTotal(); });
        total.Should().Be(100);
    }
    ...
    public async Task AddRecord_LossRecord_AddsPositiveExperience() {
        var id = Guid.NewGuid();
        var grain = GetGrain<IUserProgression>(id);
        await RunTransaction(() => grain.AddRecord(new UserProgressionRecords.Loss { Date = DateTime.UtcNow, Experience = 30 }));
        var total = 0;
        await RunTransaction(async () => { total = await grain.GetTotal(); });
        total.Should().Be(30);
    }
...
public class UserAuthTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    ...
    public async Task IsExists_FreshGrain_ReturnsFalse() {
        ...
        var exists = false;
        await RunTransaction(async () => { exists = await grain.IsExists(); });
    ...
    public async Task OnRegistered_SetsRegistrationDate() {
        ...
        await RunTransaction(async () => { date = await grain.GetDate(); });
...
public class UserDeckTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    ...
    public async Task Initialize_CreatesMaxDecksWithBaseDeck() {
        ...
        await RunTransaction(async () => { state = await grain.GetState(); });
    ...
    public async Task GetSelected_AfterInitialize_ReturnsBaseDeck() {
        ...
        await RunTransaction(async () => { selected = await grain.GetSelected(); });
    ...
    public async Task Update_SingleDeck_PersistsCards() {
        ...
        var grain = GetGrain<IUserDeck>(id);
        await RunTransaction(() => grain.Initialize());
        var customCards = new List<CardType> { CardType.Bloodhound, CardType.Bloodhound, CardType.Smoke, CardType.Smoke, CardType.OpponentBomb, CardType.OpponentBomb };
    ...
    public async Task Update_AllDecksAndSelectedIndex_Persisted() {
        ...
        var grain = GetGrain<IUserDeck>(id);
        await RunTransaction(() => grain.Initialize());
        var customCards = new List<CardType> { CardType.Smoke, CardType.Smoke, CardType.Smoke, CardType.Smoke, CardType.Smoke, CardType.Smoke };
        var decks = new Dictionary<int, IReadOnlyList<CardType>> { [0] = customCards, [1] = customCards, [2] = customCards };
...
[Collection(nameof(OrleansIntegrationCollection))]
public class UserMatchHistoryTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    [Fact]
    public async Task Add_SingleMatch_StoredInHistory() {
        ...
        await RunTransaction(async () => { block = await grain.GetBlock(10); });
    ...
    public async Task GetBlock_ReturnsLastNMatches() {
        ...
        IReadOnlyList<MatchOverview>? block = null;
        await RunTransaction(async () => { block = await grain.GetBlock(3); });
    ...
    public async Task GetBlock_MoreThanTotal_ReturnsAll() {
        ...
        await RunTransaction(() => grain.Add(CreateOverview(id, Guid.NewGuid())));
        IReadOnlyList<MatchOverview>? block = null;
        await RunTransaction(async () => { block = await grain.GetBlock(100); });
...
public class MatchTests(OrleansTestClusterFixture fixture) : IntegrationTestBase<OrleansTestClusterFixture>(fixture) {
    ...
    public async Task OnComplete_UpdatesWinnerProgression() {
        ...
        await RunTransaction(() => match.OnComplete(user1));
        var total = 0;
        await RunTransaction(async () => { total = await GetGrain<IUserProgression>(user1).GetTotal(); });
        total.Should().Be(100);
    }
    ...
    public async Task OnComplete_UpdatesLoserProgression() {
        ...
        await RunTransaction(() => match.OnComplete(user1));
        var total = 0;
        await RunTransaction(async () => { total = await GetGrain<IUserProgression>(user2).GetTotal(); });
        total.Should().Be(30);
    }
    ...
    public async Task OnComplete_UpdatesWinnerRating() {
        ...
        await RunTransaction(() => match.OnComplete(user1));
        var total = 0;
        await RunTransaction(async () => { total = await GetGrain<IUserRating>(user1).GetTotal(); });
        total.Should().Be(25);
    }
    ...
    public async Task OnComplete_UpdatesLoserRating() {
        ...
        await RunTransaction(() => match.OnComplete(user1));
        var total = 0;
        await RunTransaction(async () => { total = await GetGrain<IUserRating>(user2).GetTotal(); });
        total.Should().Be(-15);
    }
    ...
    public async Task OnComplete_AddsMatchToHistories() {
        ...
        await RunTransaction(async () => { history1 = await GetGrain<IUserMatchHistory>(user1).GetBlock(10); });
        await RunTransaction(async () => { history2 = await GetGrain<IUserMatchHistory>(user2).GetBlock(10); });
