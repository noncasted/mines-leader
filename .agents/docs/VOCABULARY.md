...
| Concept | Term | Description |
|---------|------|-------------|
| Cluster deploy identity | DeployId | Random Guid generated by Coordinator per startup; key for epoch-scoped grains |
| Deploy source of truth | IDeployManagement | Persistent grain (keyed by DeployId) with LastHeartbeat + CoordinatorReady |
| Deploy members store | IServiceDiscoveryStorage | Persistent grain (keyed by DeployId); `Update(overview)` does upsert + stale prune + returns snapshot |
| Deploy identity broadcaster | DeployIdentity | Coordinator-only hosted service: generates DeployId, binds pipe, heartbeats |
