...
| Lockdown | 3 | Противник | -- | Уменьшает макс. ходы противника на 1 на 2 раунда |
| Sonar | 3 | Своё поле | 4 | Флажит все мины в области, не открывая клетки |
| Purge | 2 | Себя | -- | Снимает все вражеские эффекты со своего поля |
...
Уменьшает максимальное количество ходов противника на `MovesReduction` на 2 раунда. По истечении автоматически восстанавливает исходное значение. Параметры настраиваются в конфиге.

### Sonar
> Мана: 3 | Цель: своё поле | Паттерн: ромб (размер 4)

Автоматически ставит флаги на все незафлаженные мины в области, не открывая клетки. Чистая разведка без риска — флажит только `HasMine && !IsFlagged`.

### Purge
> Мана: 2 | Цель: себя | Без позиции

Снимает все активные эффекты (Smoke, Fog) с каждой клетки своего поля. Контр-карта против Smoke и Fog of War.
...
| Цель | Описание | Карты |
|------|----------|-------|
| `OwnBoard` | Своё минное поле | Bloodhound, Erosion Dozer, Zip Zap, Minefield Scout, Sonar |
| `OpponentBoard` | Поле противника | Trebuchet, Opponent Bomb, Flag Erase, Flag Reshuffle, Smoke, Chain Reaction, Fog of War |
| `Self` | На себя, без позиции | Trebuchet Aimer, Gravedigger, Medic, Overclock, Scavenger, Purge |
| `Opponent` | На противника, без позиции | Siphon, Hand Scramble, Lockdown |
