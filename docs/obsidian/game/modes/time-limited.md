...
4. Начиная со **второго раунда** — если на доске игрока все оставшиеся мины отмечены флагами и нет ложных флагов, он побеждает (`RoundPlayers.GetFlagWinner`). Подорванные мины считаются зачищенными и победу по флагам не блокируют, пока HP > 0.
5. Терминирован lifetime матча → игра заканчивается без победителя (`Guid.Empty`).

Причина победы логируется отдельно (`GetWinReason`): `health reached 0`, `disconnected`, `ran out of time`, `Player <id> flagged every mine on own board`.
