...
2026-09-10. Аллокации Build урезаны ещё на треть. (1) `_exports` у сгенерированного класса — статический `Dictionary<Type, int>` и `GetExport(int)` со switch по полям: скоуп больше не создаёт свой `Dictionary<Type, object>`. (2) `ServiceRegistration` хранит `ContainerBuilder` вместо `IBuilder` и флага заморозки; `Lifetime`/`IsExisting` сняты (и `IServiceRegistration.Lifetime`) — рантайм их не читал, генератор берёт из исходника. `ContainerBuilder._injections` ленивый. (3) Диагностика в сгенерированном конструкторе и статические `_registrationInfos`/`_buildOrder` — под `#if UNITY_EDITOR || DEBUG` и флагом `ContainerRegistryDebug.IsEnabled`; `GeneratedScopes.AttachDebug` тоже под `#if`. Бенчмарк Generated меряется с выключенной диагностикой. Build 11056 → 7524 байт (VContainer 32352), Build + резолв 7708 против 45060; 10k singleton 1.44 → 1.51 мс из-за лишнего switch. Проверка: Verify ALL PASSED, 9 сборок вне Unity без ошибок, diff сгенерированного кода — только экспорты и `#if`; Internal.Tests 40/40. Плей-мод не проверен.

2026-09-10. Реальные скоупы карт замерены в плей-моде (матч против бота, временный `ProfilerRecorder` в `EntityScopeLoader`, после замера убран; диагностика в редакторе включена). На карту, минимум по замерам: соперника 5456 байт (билдер + installer 2948, `RootId` 474, `RegisterInstance` + `CreateEntity` 1906, привязка 128), своя 12854 (installer 9144, `RootId` 470, создание 3112, привязка 128). Из 9144 installer'а своей карты 4764 — `CardConfigOptions.All` в `CardFactory.BuildLocal`: свойство при каждом обращении строит новый `Dictionary` на 68 конфигов (замер `execute_code`). То же свойство зовут `CardDataView` (на каждую вьюху карты), `MenuDeckPoolCard` и бэкенд (`CardUseCommand`, `BotCardAction`, `MirrorMatch`). `All` теперь кешируется в поле при первом обращении (`CreateAll`, поле с `[JsonIgnore]`/`[MemoryPackIgnore]`); заменённый сеттером после первого обращения конфиг `All` не увидит (единственное такое место — `FrostTests`, а `Frost` читает `Frost_Normal` напрямую). Бэкенд Tests.Game 726/726; кластер нужно перезапустить. `GeneratedScopes.RootId` и `VariantKey` кешируются (ключ корня — `MethodHandle.Value`): повторный вызов 0 байт, раньше ~470 + строка варианта на каждую загрузку сущности.
