...
namespace Unity.Web.Stripping.Editor
{
    ...
    class WasmOpt
    {
        ...
        private string EnableEmscripten4FeaturesArg
        {
            get
            {
                // Правка встроенной копии пакета: без --enable-exception-handling binaryen считает, что вызовы
                // не бросают, и -O3 выкидывает все try/catch из билда с -fwasm-exceptions. Любое исключение
                // (даже пойманное в C#, например OperationCanceledException при отмене поиска) роняет плеер.
                if (EnableEmscripten4Features && EnableSimdSupport)
                    return " --enable-bulk-memory --enable-bulk-memory-opt --enable-nontrapping-float-to-int --enable-exception-handling -n";
                else if (EnableEmscripten4Features && !EnableSimdSupport)
                    return " --enable-exception-handling -n";
                else
                    return "";
            }
        }
