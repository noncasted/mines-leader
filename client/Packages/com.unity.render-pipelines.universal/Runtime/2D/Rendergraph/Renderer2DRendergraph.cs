    ...
    internal sealed partial class Renderer2D : ScriptableRenderer
    {
        ...
        ImportResourceSummary GetImportResourceSummary(RenderGraph renderGraph, UniversalCameraData cameraData)
        {
            ...
            else
            {
                ...
                output.importInfo.width = cameraData.backbufferWidth;
                output.importInfo.height = cameraData.backbufferHeight;
        ...
        void CreateOffscreenUITexture(RenderGraph renderGraph, in RenderTextureDescriptor descriptor)
        {
            ...
            DrawScreenSpaceUIPass.ConfigureOffscreenUITextureDesc(ref textureDesc, cameraData);
        ...
        public override void OnBeginRenderGraphFrame()
        {
            ...
            ApplyPixelPerfectCameraDataOverride(cameraData);

            InitGlobalShaderVars(cameraData);
        }

        void ApplyPixelPerfectCameraDataOverride(UniversalCameraData cameraData)
        {
            // Pixel Perfect Camera doesn't support camera stacking.
            if (cameraData.renderType != CameraRenderType.Base || !cameraData.resolveFinalTarget)
                return;
        ...
        void InitGlobalShaderVars(UniversalCameraData cameraData)
        {
            Universal2DGlobalShaderData shaderData = frameData.Create<Universal2DGlobalShaderData>();
            shaderData.useConstantBuffers = UniversalRenderPipeline.usePersistentConstantBuffers;
        ...
        private void OnAfterRendering(RenderGraph renderGraph)
        {
            ...
            // FSR1 also requires the final pass (unlike upscalers using IUpscaler, it does not self-schedule).
