...
URP_LIGHT_ACCUM3 SampleScreenSpaceGI(float2 pos, float invPreExposureMultiplier)
{
    ...
    return LOAD_TEXTURE2D_X(_ScreenSpaceIrradiance, pos).rgb * invPreExposureMultiplier;
}
#endif
...
// Derived both ways so shaders that still read or declare it keep working; nothing else should reference it.
#if !defined(_MIXED_LIGHTING_SUBTRACTIVE) && defined(LIGHTMAP_SHADOW_MIXING) && !defined(SHADOWS_SHADOWMASK)
    #define _MIXED_LIGHTING_SUBTRACTIVE
#elif defined(_MIXED_LIGHTING_SUBTRACTIVE) && !defined(LIGHTMAP_SHADOW_MIXING) && !defined(SHADOWS_SHADOWMASK)
...
#if (defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2))
// APV reads its data from StructuredBuffers (SSBOs). On GLES, vertex-stage SSBO support is
// device-dependent (GL_MAX_VERTEX_SHADER_STORAGE_BLOCKS may be 0), so referencing APV data in
// the vertex shader can fail to link. On GLES3, APV is evaluated per-pixel instead.
...
#if (defined(EVALUATE_SH_VERTEX) || defined(EVALUATE_SH_MIXED)) && !defined(SHADER_API_GLES3)
    half3 bakedGI;
...
half3 SampleLightmap(float2 staticLightmapUV, half3 normalWS)
{
    ...
    return result;
        ...
        #define SAMPLE_GI(irradianceTex, pos, normal, invPreExposureMultiplier) SampleScreenSpaceGI(pos, invPreExposureMultiplier)
    #else
        #define SAMPLE_GI(irradianceTex, pos, normal, invPreExposureMultiplier) EvaluateAmbientProbe(normal)
    #endif
...
// Prevent calling 'reflect' from causing NdotV to be computed twice on Adreno GPUs
    ...
    URP_LIGHT_ACCUM3 indirectDiffuse = bakedGI;
