...
namespace Internal {
    public static class SpriteAddressablesSync {
        internal static void Sync(IReadOnlyList<SpriteGroupDefinition> groups) {
            var settings = AddressableAssetSettingsDefaultObject.Settings;
            if (settings == null) {
                Debug.LogError("[SpriteAddressablesSync] Addressable settings are missing.");
                return;
            }

            var usedGroups = new HashSet<string>(StringComparer.Ordinal);
            foreach (var group in groups) {
                var assetPath = $"{SpriteGenerator.GroupsFolder}/{group.Name}.asset";
        ...
        private static string MarkGroupAsset(AddressableAssetSettings settings, string assetPath, string groupName) {
            ...
            if (entry == null) {
                Debug.LogError($"[SpriteAddressablesSync] Failed to mark {assetPath} addressable.");
                return guid;
            }
        ...
        private static void RemoveUnusedSpriteGroups(AddressableAssetSettings settings, HashSet<string> usedGroups) {
            var stale = new List<AddressableAssetGroup>();
            foreach (var group in settings.groups) {
                if (group == null || group.Name.StartsWith("Sprites_", StringComparison.Ordinal) == false)
                    continue;
