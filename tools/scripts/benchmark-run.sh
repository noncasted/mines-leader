...
if [ "$MODE" = "group" ]; then
  curl -s -X POST "${CONSOLE_URL:-http://localhost:7103}/api/benchmarks/group/${TARGET}/run" --max-time 600
elif [ "$MODE" = "single" ]; then
  ENCODED=$(python3 -c "import urllib.parse; print(urllib.parse.quote('${TARGET}'))")
  curl -s -X POST "${CONSOLE_URL:-http://localhost:7103}/api/benchmarks/${ENCODED}/run" --max-time 120
fi
